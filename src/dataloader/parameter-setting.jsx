...
import { cookie_path, Local_state_path, cookie_page_setting, appCacheDir_path } from './parameter-cookies'
...
import { downloadfile_savepath } from './parameter-download'
// 定義參數
const setting_table = []
// 選項輸出 [類型,名稱,解釋,功能]
...
const downloader_savepath = signal(
    [
        'Dir',
        'Download code path ',
        '',
        <div className='setting-handle'>
            <div className='setting-show' >
                <CopyButton target={appCacheDir_path} />
            </div >
            <div className='setting-control'>
                <button className='control-btn' id='path-selecter' onClick={async () => {
                    const selected = await select_single({ isdir: true, defaultP: appCacheDir_path.value })
                    appCacheDir_path.value = selected || appCacheDir_path.value;
                }}>Select cookie path</button >
                <button className='control-btn' id='path-reset' onClick={async () => {
                    localStorage.removeItem('cookies-path');
                    appCacheDir_path.value = await appCacheDir();
                }}>Reset cookie path</button >
            </div>
        </div>
    ])
...
const download_file_savepath = signal(
    [
        'Dir',
        'Download file path ',
        '',
        <div className='setting-handle'>
            <div className='setting-show' >
                <CopyButton target={downloadfile_savepath} />
            </div >
            <div className='setting-control'>
                <button className='control-btn' id='path-selecter' onClick={async () => {
                    const selected = await select_single({ isdir: true, defaultP: downloadfile_savepath.value })
                    downloadfile_savepath.value = selected || downloadfile_savepath.value;
                }}>Select cookie path</button >
                <button className='control-btn' id='path-reset' onClick={async () => {
                    localStorage.removeItem('cookies-path');
                    downloadfile_savepath.value = await appCacheDir();
                }}>Reset cookie path</button >
            </div>
        </div>
    ])
...
setting_table.push(downloader_savepath.value);
setting_table.push(download_file_savepath.value);

setting_table.sort();

export { setting_table, appCacheDir_path }
